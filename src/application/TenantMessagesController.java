...
public class TenantMessagesController {
    ...
    private void loadConversations() {
        ...
        List<Application> apps = DatabaseHandler.getTenantConversations(myId);

        if (apps.isEmpty()) {
            Label placeHolder = new Label("No active applications.");
    ...
    private HBox createConversationItem(Application app) {
        ...
        Label nameLbl = new Label(app.getLandlordName() != null ? app.getLandlordName() : "Unknown Landlord");
        nameLbl.getStyleClass().add("conv-name");

        Pane spacer = new Pane();
        HBox.setHgrow(spacer, Priority.ALWAYS);

        Label timeLbl = new Label(dateSdf.format(app.getApplyDate())); // Use apply date or last msg date if available
    ...
    private void selectConversation(Application app, HBox row) {
        ...
        // Update Header
        lblChatName.setText(app.getLandlordName());
        lblPropertyTitle.setText(app.getPropertyName());
    ...
    private void handleSend() {
        ...
        boolean success = DatabaseHandler.saveMessage(currentApp.getId(), myId, currentApp.getLandlordId(), text);
        if (success) {
            txtMessage.clear();
            loadMessages(currentApp); // Refresh to see sent message
        }
    }
