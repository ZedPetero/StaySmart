...
public class HouseViewController {
    ...
    private void buildHouseVisuals(List<Floor> floors) {
        ...
        boolean isRural = "Rural".equalsIgnoreCase(currentProperty.getType());

        StackPane roof = new StackPane();
        String roofClass = isRural ? "rural-roof" : "urban-roof";
        roof.getStyleClass().add(roofClass);
        buildingVBox.getChildren().add(roof);

        String type = isRural ? "rural" : "urban";
        for (Floor f : floors) {
            buildingVBox.getChildren().add(createFloorContainer(f.getLevel(), floors.size(), floors, type));
        }

        Pane base = new Pane();
        String baseClass = isRural ? "rural-base" : "urban-base";
    ...
    private void openApplicationDialog(Room room) {
        if (room.getStatus() == null || room.getStatus().equalsIgnoreCase("Unconfigured") || room.getStatus().equals("New"))
            return;
        if (room.getStatus().equalsIgnoreCase("Occupied") || room.getStatus().equalsIgnoreCase("Maintenance")) {
            javafx.scene.control.Alert alert = new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.INFORMATION);
            alert.setTitle("Room unavailable");
            alert.setHeaderText(null);
            alert.setContentText("Room " + room.getRoomNumber() + " is currently " + room.getStatus().toLowerCase() + " and cannot be applied for.");
            alert.showAndWait();
            return;
        }
        try {
    ...
    private void openRoomEditor(int floorLevel, Room room) {
        try {
            ...
            AppWindow.applyIcon(dialogStage);
    ...
    private VBox createRoomDetailHoverView(Room room) {
        ...
        Label price = new Label("₱ " + String.format("%,.2f", room.getPrice() != null ? room.getPrice() : 0.0) + " / month");
        price.setStyle("-fx-text-fill: #27ae60; -fx-font-weight: bold;");
        card.getChildren().add(price);
        HBox pills = new HBox(8);
        Label statusLbl = new Label(room.getStatus() != null ? room.getStatus() : "Available");
        statusLbl.getStyleClass().add("status-pill");
        if ("Occupied".equalsIgnoreCase(room.getStatus()) || "Maintenance".equalsIgnoreCase(room.getStatus()))
            statusLbl.setStyle(
